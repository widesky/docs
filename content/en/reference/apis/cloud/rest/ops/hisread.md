...
The hisRead operation is used to read time series data from one or more historised points. You can perform a query on a single point, or read from multiple points using our extended multi-point query. The hisRead operation also supports time zone conversion including daylight savings.
...
- **Request Data Params (single point):** A grid with a single row and the following columns:
  ...
  |`range`|✓|`Str`|The time [range](./#ranges) to retrieve from time series storage.|
...
- **Request Data Params (multiple points):** A grid with a single row and the following columns:
  ...
  |…||…|… etc …|
  |`idN`||`Ref`|The nth `point` to read history records from.|
...
- **Response (single point):** Grid with the following metadata fields:
  ...
  |`hisStart`|`DateTime`|Timestamp for range start in `point`'s time zone|
  |`hisEnd`|`DateTime`|Timestamp for range end in `point`'s time zone|
...
## Ranges
The `range` parameter is a string with one of the following formats:
...
|`{DateTime}`|All of the samples at and following the timestamp given by `{DateTime}`|
|`{DateTime},{DateTime}`|All of the samples between the two timestamps starting at and including the start `{DateTime}` and finishing *before* (and excluding) the end `{DateTime}`|
|`{Date}`|All of the samples from midnight of the date defined to midnight of the day following the defined date|
|`{Date},{Date}`|All of the samples that fall on or after the first `{Date}` and on or before the second `{Date}`|

… where:

* `{Date}` is a Date value (e.g. `2015-07-24`)
* `{DateTime}` is a Date and Time value (e.g. `2015-07-24T10:34:15+10:00 Brisbane`)
...
When specifying `DateTime` ranges (that is, ranges of the form `${DateTime},${DateTime}`), the time range selected is **inclusive** of the start `DateTime` value and **exclusive** of the end `DateTime` value.  When the range uses `Date`s instead, the value is inclusive of *both* date values given.  For example:

|Range|Literal Example|Effective range|
|--------------|---------------|---------------|
|`{DateTime}`|`2020-04-27T10:00+10:00 Brisbane`|Any sample taken exactly at or after 10:00AM AEST on the 27th April|
|`{DateTime},{DateTime}`|`2020-04-27T08:30+10:00 Brisbane,2020-04-27T17:00+10:00 Brisbane`|All samples taken between 8:30AM and 5:00PM on the 27th April **including** exactly 8:30:00.000 AM right up to 4:59:59.999 PM but **excluding** 5:00:00.000 PM.|
|`{Date}`|`2020-04-27`|All samples taken immediately at midnight or after AEST on the 27th, right up to 11:59:59.999 PM that same day, **excluding** exactly midnight AEST on the 28th.|
|`{Date},{Date}`|`2020-04-27,2020-05-01`|All samples taken immediately at midnight or after AEST on the 27th, right up to 11:59:59.999 PM on the 1st, **excluding** exactly midnight AEST on the 2nd.|
...
### Requests *without* time zone specified
...
If one or more points however has a *different* time zone, the [`Rel` time zone](https://fantom.org/doc/sys/time zone#rel) is used.  This time zone has a 0-minute offset from UTC, and is used to indicate that all data in the response has been *normalised* to this common time zone.  For example:
...
### Sinlge point with DST transition
...

  {
    ...
    "rows": [
      {
        "id0": "r:13641c0a-8e88-41b5-b3ed-75240d8b9bb2",
        "range": "s:2000-03-26T02:00:00+11:00 Sydney, 2000-03-26T02:01:00+10:00 Sydney"
      }
    ]
  }
  ```
...
### Multiple points in differnet time zones, and time zone omitted
...

  {
    ...
    "rows": [
      {
        "id0": "r:00000111-0000-0000-0000-000000000000",
        "id1": "r:00000112-0000-0000-0000-000000000000",
        "range": "s:2016-01-01"
      }
    ]
  }
